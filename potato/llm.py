...
async def achat(
    ...
    *,
    ...
    """Async version of chat() — uses httpx.AsyncClient to avoid blocking the event loop."""
    if task not in TASK_TYPES:
        logger.warning("Unknown task type '%s', falling back to 'fallback'", task)
        task = "fallback"
    ...
    for provider in providers:
        ...
        if provider.get("provider_type") == "base44":
            result = await _acall_base44(provider, key, system, user_prompt, max_tokens, task, errors)
            if result is not None:
                return result
            continue
        ...
        for attempt in range(_MAX_RETRIES):
            try:
                ...
                _mark_success(provider["name"])
                logger.info("LLM achat success from %s (model=%s, task=%s)", provider["name"], model, task)
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=_TIMEOUT) as client:
            ...
            if not content.strip():
                ...
                return None
            try:
                await client.delete(f"{base_url}/conversations/{conv_id}", headers=headers)
            except Exception:
                pass
        _mark_success(provider["name"])
        logger.info("LLM achat success from %s (base44 agent, task=%s)", provider["name"], task)
        return {
            "ok": True,
            "content": content.strip(),
...
async def aresearch(prompt: str, *, system: str = "", settings: Settings | None = None, max_tokens: int = 2000) -> dict[str, Any]:
    return await achat(prompt, system=system or "你是专业金融研究员，提供有来源依据的深度分析。用中文输出，引用数据要标注来源。", settings=settings, max_tokens=max_tokens, task="research", use_json=False)


async def aanalyze(prompt: str, *, system: str = "", settings: Settings | None = None, max_tokens: int = 1500) -> dict[str, Any]:
    return await achat(prompt, system=system or "你是保守型A股操盘分析师，用简洁中文输出结构化JSON。", settings=settings, max_tokens=max_tokens, task="analysis", use_json=True)
